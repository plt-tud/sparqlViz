...
angular.module("sparqlJs").controller("graphController", function ($scope, highlightingService, dragNodeService, parseQueryService, $location, sparqlProxyService, $interval) {
    $scope.nodes = [];
    $scope.edges = [];
    $scope.subGraphs = [];
    $scope.filters = [];
    ...
    $scope.$watch("queries.sparqlRequests", function (newVal, oldVal, scope) {
        ...
        force.on('tick', function () {
            for (var _i = 0, _a = $scope.nodes; _i < _a.length; _i++) {
                var n = _a[_i];
            ...
            for (var _b = 0, _c = $scope.edges; _b < _c.length; _b++) {
                var e = _c[_b];
                e.updateCenterPoint();
